...
const CPPAnnouncementPage = () => {
  ...
  useEffect(() => {
    const getCPPAnnouncements = async (e) => {
      try {
        const response = await fetch(
          'http://localhost:8000/courses/announcements',
          {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              courseName: 'CPP Programming Language',
            }),
          }
        )
    ...
    <div className="w-[100vw] min-h-[100vh] flex relative">
      ...
      <div className="right_sidebar container p-8 px-12">
        ...
        <div class="p-6 px-6 ms-auto w-[85%]">
          ...
          <p class="text-xl pt-8 text-[#131313]">Announcements</p>
          {announcements
            ? announcements?.announcement?.map((ann) => {
                return (
                  <>
                    <hr class="my-3 text-[#c6c6c6]" />
                    <p class="text-sm text-[#131313] font-bold">{ann.title}</p>
                    <p class="text-sm text-[#131313] my-2">{ann.description}</p>
                  </>
                )
              })
...
export default CPPAnnouncementPage
